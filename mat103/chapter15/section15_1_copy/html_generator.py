...
# === Settings ===
input_md = "source_test.md"
output_root = Path("output_test")
...
for i, page in enumerate(pages, start=1):
    ...
    def replace_ans_blocks(text):
        ...
        def _repl(match):
            ...
            return f"""
<button id="show-ans-{counter}" class="btn btn-primary">Show</button>
<div id="ans-div-{counter}" style="display: none;">
{inner}
</div>
  <script>
    document.getElementById("show-ans-{counter}").addEventListener("click", function() {{
      const ans = document.getElementById("ans-div-{counter}");
      const btn = document.getElementById("show-ans-{counter}");
